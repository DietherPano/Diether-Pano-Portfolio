export default function SkillsSection({ skillCards }) {
  return (
    <section id="skills" className="reveal-up space-y-5" style={{ '--reveal-delay': '100ms' }}>
      <div className="space-y-1">
        <h2 className="dev-section-title text-2xl font-semibold text-slate-50 sm:text-3xl">Skills</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {skillCards.map((group, index) => (
          <article
            key={group.title}
            className="reveal-up space-y-4 rounded-2xl border border-slate-800/70 bg-slate-900/45 p-5"
