    ...
    <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-800/70 bg-slate-950/85 backdrop-blur">
      <div className="mx-auto w-full max-w-6xl px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          <a href="#home" className="flex min-w-0 items-center gap-3" aria-label="Go to top" onClick={closeMenu}>
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <button
              ...
              className="ml-1 inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 text-slate-200 transition hover:border-cyan-300/60 hover:text-cyan-200 md:hidden"
              onClick={toggleMenu}
            >
              {menuOpen ? (
        ...
        <nav
          ...
          className={`mt-3 overflow-hidden rounded-xl border border-slate-800 bg-slate-900/95 text-sm text-slate-200 transition-all duration-300 ease-out md:hidden ${
            menuOpen
